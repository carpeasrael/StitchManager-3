...
// ── Custom Thumbnail Upload (#120) ───────────────────────────────────

const THUMB_IMAGE_EXTENSIONS: &[&str] = &["png", "jpg", "jpeg", "bmp", "gif", "webp"];
...
pub fn upload_thumbnail(
    ...
    if !THUMB_IMAGE_EXTENSIONS.contains(&ext.as_str()) {
        return Err(AppError::Validation(format!(
            "Nicht unterstuetztes Bildformat: .{ext}. Erlaubt: PNG, JPG, BMP, GIF, WEBP"
        )));
    }
    ...
    thumb.write_to(&mut cursor, image::ImageFormat::Png)
        .map_err(|e| AppError::Internal(format!("PNG-Encoding Fehler: {e}")))?;
